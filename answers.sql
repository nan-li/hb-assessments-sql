...
customer_id = 
(SELECT id FROM customers WHERE fname='Jane' AND lname='Doe');
...
SELECT SUM(num_cupcakes) FROM
orders WHERE processed=FALSE;
...
FROM cupcakes LEFT JOIN orders
...
(SELECT customer_id FROM orders GROUP BY customer_id HAVING SUM(num_cupcakes)>100
)
ORDER BY email DESC;
...
SELECT DISTINCT fname, lname, email FROM
customers JOIN orders ON (customer_id=customers.id)
